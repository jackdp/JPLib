...

{
  ...
  Rozwiązanie niewiele wolniejsze niż z zastosowaniem rekurencji, za to mniej pamięciożerne.

  Moduł z 2002 roku. W kolejnych latach rozbudowywany.
  ...
  Uwaga!
  FindFiles
  FPC 3.1.1 ma problemy z maskami w FindFirst/FindNext. Czasami akceptuje pliki zupełnie nie pasujące do maski!
  Na Delphi XE7 wszystko działa prawidłowo.
  W FPC wprowadziłem dodatkowe sprawdzenie przez MatchesMask.
...
var
  ...
  JPFileSearchCaseSensitive: Boolean; // { TODO: Dodać CaseSensitive do funkcji JPGetFileList, JPGetDirectoryList, JPGetFileList2 }
{$ENDIF}
...
begin
  ...
  // FPC 3.1.1: FindFirst czasami "przepuszcza" pliki nie pasujące do maski, dlatego
  // w FindFirst ustawiam maskę na '*', a filtrowanie wykonuję przy pomocy MatchesMask.
...
{
  Procedura JPGetFileList najpierw tworzy listę katalogów w MainDirectoryList, a później listę plików dla każdego znalezionego katalogu.
}
procedure JPGetFileList(
  FileMask, StartDir: string; var FileList: TStringList; RecurseDepth: integer = DEFAULT_RECURSE_DEPTH;
  AcceptDirSymLinks: Boolean = True; AcceptFileSymLinks: Boolean = True;
...
begin
  ...
  try
    ...
    if FileList.Count < 10000 then Filelist.Sort; // przy dużej liczbie plików sortowanie trwa bardzo długo
