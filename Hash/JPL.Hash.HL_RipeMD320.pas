...
uses
  ...
  HlpHash, HlpHashResult, HlpIHashResult, HlpRIPEMD320;
...
function HlGetFileHash_RipeMd320(fName: string; var HashResult: THashResultRec; HashEnumProc: THashEnumProc = nil): Boolean;
function HlGetStreamHash_RipeMd320(AStream: TStream; var HashResult: THashResultRec; StartPos: Int64 = 0; HashEnumProc: THashEnumProc = nil): Boolean;
...
begin
  ...
  Hash := TRIPEMD320.Create;
  try
    Logger.StartLog;
    ClearHashResultRec(HashResult);
    HashResult.HashType := htRipeMD320;
...
begin
  ...
  try
    Result := HlGetStreamHash_RipeMd320(fs, HashResult, 0, HashEnumProc);
  finally
    fs.Free;
  end;
end;
