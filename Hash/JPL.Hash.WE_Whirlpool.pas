...
uses
  ...
  Hash, Mem_util, Whirl512
  ;

const
  HASH_BUFFER_SIZE_WHIRLPOOL = 1024*63; // 1024 * 100; // F000 = 61 440 bytes


function WeGetFileHash_Whirlpool(fName: string; var HashResult: THashResultRec; HashEnumProc: THashEnumProc = nil): Boolean;
function WeGetStreamHash_Whirlpool(AStream: TStream; var HashResult: THashResultRec; StartPos: Int64 = 0; HashEnumProc: THashEnumProc = nil): Boolean;
...
var
  ...
  BufNo: integer;
  Buffer: array[0..HASH_BUFFER_SIZE_WHIRLPOOL-1] of Byte;
...
begin
  ...
  try
    ...
    HashResult.HashType := htWhirlpool;
    Whirl_Init(Context);
    ...
    begin
      ...
      if xRead > 0 then Whirl_Update(Context, @Buffer, xRead);
...
begin
  ...
  try
    Result := WeGetStreamHash_Whirlpool(fs, HashResult, 0, HashEnumProc);
  finally
    fs.Free;
  end;
end;
