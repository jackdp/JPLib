...
uses
  ...
  HlpHash, HlpIHashResult, HlpHashSize, HlpSnefru;
...
function HlGetFileHash_Snefru128(fName: string; var HashResult: THashResultRec; HashEnumProc: THashEnumProc = nil): Boolean;
function HlGetStreamHash_Snefru128(AStream: TStream; var HashResult: THashResultRec; StartPos: Int64 = 0; HashEnumProc: THashEnumProc = nil): Boolean;
...
begin
  ...
  Hash := TSnefru.Create(8, THashSize.hsHashSize128 {hsHashSize128}); // Snefru Security Level Cannot be Less than 1. Standard Level is 8
  try
    Logger.StartLog;
    ClearHashResultRec(HashResult);
    HashResult.HashType := htSnefru128;
...
begin
  ...
  try
    Result := HlGetStreamHash_Snefru128(fs, HashResult, 0, HashEnumProc);
  finally
    fs.Free;
  end;
end;
